    ...
    fileprivate let tokenStores: [Store] = {
        return tokens.map {
            ...
            if $0.code == "TBRD" {
                ...
                let crowdSale = Crowdsale(startTime: nil, endTime: nil, minContribution: nil, maxContribution: nil,contract: Contract(address: "0x5250776FAD5A73707d222950de7999d3675a2722", abi: crowdSaleABI), rate: nil, verificationCountryCode: nil, weiRaised: nil, cap: nil)
                store.perform(action: WalletChange.set(store.state.walletState.mutate(crowdSale: crowdSale)))
            }  else if $0.code == "BRd" {
